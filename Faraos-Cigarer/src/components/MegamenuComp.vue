...
<template>
  <div class="megamenu-container" aria-label="Megamenu">
    <button
      class="cat-button"
      :class="{ 'cat-button-open': isOpen }"
      @click="toggleMenu"
      aria-label="Åbn eller luk megamenu"
    ...
    >
      ...
      <img
        class="cat-arrow"
        src="../assets/icons/arrow-vector-icon.svg"
        alt="Pil peger op"
    ...
    <div
      id="megamenu-panel"
      class="megamenu"
      :class="{ 'megamenu-open': isOpen }"
      aria-label="Kategorivalg panel"
      :aria-hidden="!isOpen"
    >
      <div class="category-buttons" aria-label="Kategoriknapper">
        <button @click="loadAllBooks" aria-label="Vis bøger og tegneserier">
          BØGER & TEGNESERIER
        </button>
        <button @click="loadAllBooks" aria-label="Vis retro produkter">
          RETRO
        </button>
        <button @click="loadAllBooks" aria-label="Vis merchandise">
          MERCHANDISE
        </button>
        <button @click="loadAllBooks" aria-label="Vis spil">SPIL</button>
        <button @click="loadAllBooks" aria-label="Vis udklædning">
          UDKLÆDNING
        </button>
      </div>
      ...
      <div
        ...
        class="shown-message"
        aria-label="Introduktionsbesked"
      >
        <h3 class="msg-title">Hvad er du på udkig efter?</h3>
        <p class="msg-text">
          Skal du vindues-shoppe, shop-amok eller bare smugkigge på alle vores
          mange produkter? Så kan du klikke på kategorierne til venstre for at
          finde lige netop det produkt du leder efter!
        </p>
        <p class="msg-text">
          <span>"You have the high ground now" - Obi Wan Kenobi</span>
        </p>
      </div>

      <div v-else class="category-content" aria-label="Kategoriindhold">
        <div
          v-for="category in currentList"
          :key="category.title"
          class="category-wrapper"
          :aria-label="'Kategori: ' + category.title"
        >
          ...
          <div class="category-links" aria-label="Underkategorier">
            <RouterLink
              v-for="item in category.items"
              :key="item.route"
              :to="item.route"
              class="link-item"
              :aria-label="'Gå til ' + item.name"
            >
              {{ item.name }}
            </RouterLink>
          </div>
